...
public:
	...
	{
	 	colors[idx] = col; positions[idx] = pos[a]; idx++;
	  	colors[idx] = col; positions[idx] = pos[b]; idx++;
	  	colors[idx] = col; positions[idx] = pos[c]; idx++;
	  	colors[idx] = col; positions[idx] = pos[a]; idx++;
	  	colors[idx] = col; positions[idx] = pos[c]; idx++;
	  	colors[idx] = col; positions[idx] = pos[d]; idx++;
	}
...
public:
	...
	void quad(int a, int b, int c, int d)
	{
		colors[idx] = col[a]; positions[idx] = pos[a]; idx++;
		colors[idx] = col[b]; positions[idx] = pos[b]; idx++;
		colors[idx] = col[c]; positions[idx] = pos[c]; idx++;
		colors[idx] = col[a]; positions[idx] = pos[a]; idx++;
		colors[idx] = col[c]; positions[idx] = pos[c]; idx++;
		colors[idx] = col[d]; positions[idx] = pos[d]; idx++;
	}
